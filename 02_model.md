**Оглавление**
- [2. Проектирование аналитического решения](#2-проектирование-аналитического-решения)
  - [2.1. Целевая модель и выбор схемы](#21-целевая-модель-и-выбор-схемы)
    - [Архитектурный подход](#архитектурный-подход)
    - [Обоснование выбора](#обоснование-выбора)
    - [ER-диаграмма](#er-диаграмма)
  - [2.2. Зерно таблиц и витрин](#22-зерно-таблиц-и-витрин)
    - [ods.po\_line\_version](#odspo_line_version)
    - [dds.fact\_po\_line](#ddsfact_po_line)
    - [dds.dim\_supplier](#ddsdim_supplier)
    - [dds.dim\_supplier\_golden](#ddsdim_supplier_golden)
    - [dds.dim\_item / dds.dim\_uom / dds.dim\_date](#ddsdim_item--ddsdim_uom--ddsdim_date)
    - [dds.ref\_fx\_rate\_daily](#ddsref_fx_rate_daily)
    - [mart.spend\_monthly](#martspend_monthly)
    - [mart.price\_benchmark](#martprice_benchmark)
    - [mart.price\_dynamics\_quarter](#martprice_dynamics_quarter)
    - [dq.run\_metric](#dqrun_metric)
    - [Где модель могла бы задвоить факты и что этому препятствует](#где-модель-могла-бы-задвоить-факты-и-что-этому-препятствует)
  - [2.3. DDL целевых таблиц](#23-ddl-целевых-таблиц)
  - [2.4. Обработка сложных случаев](#24-обработка-сложных-случаев)
    - [Историчность поставщика](#историчность-поставщика)
    - [Мультивалютность](#мультивалютность)
    - [Единицы измерения](#единицы-измерения)
    - [Изменения и отмены заказов](#изменения-и-отмены-заказов)
    - [Отсутствующая номенклатура](#отсутствующая-номенклатура)
  - [2.5. Матрица метрик](#25-матрица-метрик)
    - [Каноническое определение OTIF](#каноническое-определение-otif)
    - [Почему сейчас могут получаться 94% и 71%](#почему-сейчас-могут-получаться-94-и-71)
    - [Предлагаемый способ reconciliation](#предлагаемый-способ-reconciliation)
  - [Проверка согласованности решения](#проверка-согласованности-решения)
  - [2.6. Результаты анализа качества исходных данных](#26-результаты-анализа-качества-исходных-данных)
- [Найденные дефекты данных](#найденные-дефекты-данных)
  - [Дефект 1. Дубли и неразрешённые версии строк заказа](#дефект-1-дубли-и-неразрешённые-версии-строк-заказа)
    - [В чём дефект](#в-чём-дефект)
    - [Как обнаружили](#как-обнаружили)
    - [Как влияет на цифры в дашборде](#как-влияет-на-цифры-в-дашборде)
    - [Как обрабатываем в пайплайне](#как-обрабатываем-в-пайплайне)
    - [Приоритет](#приоритет)
  - [Дефект 2. Единицы измерения строки заказа не сопоставимы с номенклатурой](#дефект-2-единицы-измерения-строки-заказа-не-сопоставимы-с-номенклатурой)
    - [В чём дефект](#в-чём-дефект-1)
    - [Как обнаружили](#как-обнаружили-1)
    - [Как влияет на цифры в дашборде](#как-влияет-на-цифры-в-дашборде-1)
    - [Как обрабатываем в пайплайне](#как-обрабатываем-в-пайплайне-1)
    - [Приоритет](#приоритет-1)
  - [Дефект 3. Дубли поставщиков по ИНН и история, начинающаяся позже первых заказов](#дефект-3-дубли-поставщиков-по-инн-и-история-начинающаяся-позже-первых-заказов)
    - [В чём дефект](#в-чём-дефект-2)
    - [Как обнаружили](#как-обнаружили-2)
    - [Как влияет на цифры в дашборде](#как-влияет-на-цифры-в-дашборде-2)
    - [Как обрабатываем в пайплайне](#как-обрабатываем-в-пайплайне-2)
    - [Приоритет](#приоритет-2)
  - [Дефект 4. Цены в CNY несопоставимы по масштабу после конвертации](#дефект-4-цены-в-cny-несопоставимы-по-масштабу-после-конвертации)
    - [В чём дефект](#в-чём-дефект-3)
    - [Как обнаружили](#как-обнаружили-3)
    - [Как влияет на цифры в дашборде](#как-влияет-на-цифры-в-дашборде-3)
    - [Как обрабатываем в пайплайне](#как-обрабатываем-в-пайплайне-3)
    - [Приоритет](#приоритет-3)
  - [Дефект 5. Строки заказа ссылаются на отсутствующую номенклатуру](#дефект-5-строки-заказа-ссылаются-на-отсутствующую-номенклатуру)
    - [В чём дефект](#в-чём-дефект-4)
    - [Как обнаружили](#как-обнаружили-4)
    - [Как влияет на цифры в дашборде](#как-влияет-на-цифры-в-дашборде-4)
    - [Как обрабатываем в пайплайне](#как-обрабатываем-в-пайплайне-4)
    - [Приоритет](#приоритет-4)
  - [Дефект 6. Приёмки по отменённым строкам заказа](#дефект-6-приёмки-по-отменённым-строкам-заказа)
    - [В чём дефект](#в-чём-дефект-5)
    - [Как обнаружили](#как-обнаружили-5)
    - [Как влияет на цифры в дашборде](#как-влияет-на-цифры-в-дашборде-5)
    - [Как обрабатываем в пайплайне](#как-обрабатываем-в-пайплайне-5)
    - [Приоритет](#приоритет-5)
  - [Дефект 7. В справочнике курсов валют отсутствуют выходные дни](#дефект-7-в-справочнике-курсов-валют-отсутствуют-выходные-дни)
    - [В чём дефект](#в-чём-дефект-6)
    - [Как обнаружили](#как-обнаружили-6)
    - [Как влияет на цифры в дашборде](#как-влияет-на-цифры-в-дашборде-6)
    - [Как обрабатываем в пайплайне](#как-обрабатываем-в-пайплайне-6)
    - [Приоритет](#приоритет-6)
  - [Дефект 8. Приёмка датирована раньше заказа](#дефект-8-приёмка-датирована-раньше-заказа)
    - [В чём дефект](#в-чём-дефект-7)
    - [Как обнаружили](#как-обнаружили-7)
    - [Как влияет на цифры в дашборде](#как-влияет-на-цифры-в-дашборде-7)
    - [Как обрабатываем в пайплайне](#как-обрабатываем-в-пайплайне-7)
    - [Приоритет](#приоритет-7)
  - [Дефект 9. Смешение UTC и MSK меняет вердикт «в срок» на границе суток](#дефект-9-смешение-utc-и-msk-меняет-вердикт-в-срок-на-границе-суток)
    - [В чём дефект](#в-чём-дефект-8)
    - [Как обнаружили](#как-обнаружили-8)
    - [Как влияет на цифры в дашборде](#как-влияет-на-цифры-в-дашборде-8)
    - [Как обрабатываем в пайплайне](#как-обрабатываем-в-пайплайне-8)
    - [Приоритет](#приоритет-8)
  - [Дефект 10. Две конкурирующие колонки категории в справочнике номенклатуры](#дефект-10-две-конкурирующие-колонки-категории-в-справочнике-номенклатуры)
    - [В чём дефект](#в-чём-дефект-9)
    - [Как обнаружили](#как-обнаружили-9)
    - [Как влияет на цифры в дашборде](#как-влияет-на-цифры-в-дашборде-9)
    - [Как обрабатываем в пайплайне](#как-обрабатываем-в-пайплайне-9)
    - [Приоритет](#приоритет-9)
- [Наблюдения без подтверждённого статуса дефекта](#наблюдения-без-подтверждённого-статуса-дефекта)
- [Проверки, не выявившие дефектов](#проверки-не-выявившие-дефектов)
